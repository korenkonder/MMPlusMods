...
#include "../../MMPlusModsShared/auth_2d.hpp"
#include "../../MMPlusModsShared/auth_3d.hpp"
#include "../../MMPlusModsShared/print_work.hpp"
...
static_assert(sizeof(pv_game_item_mask) == 0x05, "\"pv_game_item_mask\" struct should have a size of 0x05");
...
static_assert(sizeof(pv_game_init_data) == 0xD0, "\"pv_game_init_data\" struct should have a size of 0xD0");
...
public:
    struct Args {
        ...
        bool field_1BA;
        int32_t option;
        prj::string mix_mode_left_se_name;
        prj::string mix_mode_right_se_name;
        prj::string mix_slide_loop_se_name;
        prj::string mix_slide_end_se_name;
        bool field_240;
    };

    static_assert(sizeof(TaskPvGame::Args) == 0x248, "\"TaskPvGame::Args\" struct should have a size of 0x248");
    ...
    static_assert(sizeof(TaskPvGame::Data) == 0x250, "\"TaskPvGame::Data\" struct should have a size of 0x250");

    TaskPvGame::Data data;
};

static_assert(sizeof(TaskPvGame) == 0x2B8, "\"TaskPvGame\" struct should have a size of 0x2B8");
...
TaskPvGame& task_pv_game = *(TaskPvGame*)0x000000014CBFFDC0;
TaskPvGameX* task_pv_game_x;
const firstread* firstread_ptr;
x_pv_game_firstread* x_pv_game_firstread_ptr;
pv_disp2d& pv_disp2d_data = *(pv_disp2d*)0x0000000140DAC0A0;
size_t pv_game_ptr = 0x00000001412C2330;
size_t& pv_game_controller_ptr = *(size_t*)0x000000014CC105A0;
pv_game_parent& pv_game_parent_data = *(pv_game_parent*)0x00000001412F0248;

bool reflect_full = false;

bool pv_game_load_InitScript_tail_impl() {
...
bool pv_game_load_WaitForAuthObject_mid_impl() {
    return task_pv_game_x->use ? task_pv_game_x->data.effect.state < 11 : false;
}

bool pv_game_load_WaitForAuthHandle_tail_impl() {
    if (task_pv_game_x->use) {
        bool wait_load = false;
...
HOOK(void, FASTCALL, pv_game_load_InitScript_tail, 0x000000014025C2D0);
...
HOOK(bool, FASTCALL, task_pv_game_del_task, 0x000000015A829180) {
...
HOOK(bool, FASTCALL, task_pv_game_add_task, 0x00000001405DA0D0, TaskPvGame::Args* args) {
...
HOOK(void, FASTCALL, pv_game_pv_data__dsc_reset_position, 0x0000000140254930, size_t This) {
...
HOOK(bool, FASTCALL, pv_game_pv_data__load, 0x000000014024E270, size_t This,
...
void pv_game_patch() {
    INSTALL_HOOK(pv_game_load_InitScript_tail);
...
static uint32_t(FASTCALL* get_main_timer)() = (uint32_t(FASTCALL*)())0x00000001402B7670;
...
uint32_t aet_obj_data::init(AetArgs& args) {
    ...
    id = aet_manager_init_aet_object(args, id);
    if (!id)
        return 0;
...
void x_pv_game_title::load_data() {
    ...
    prj::string empty_string;
    sprite_manager_read_file(spr_set_id, prj::string_range(empty_string));
    aet_manager_read_file(aet_set_id, empty_string);
    state = 2;
}
...
static void print_dsc_command(dsc& dsc, dsc_data* dsc_data_ptr, int64_t time) {
    static const char* (FASTCALL * dw_console_printf)(int32_t index, const char* fmt, ...)
        = (const char* (FASTCALL*)(int32_t index, const char* fmt, ...))0x00000001519E1650;
...
bool TaskPvGameX::ctrl() {
    ...
    case 2: {
        uint64_t pv_game_load_ptr = *(uint64_t*)(pv_game_ptr + 0xC8 + 0x2D318);
        if (!pv_game_load_ptr || *(int32_t*)(pv_game_load_ptr + 0x08) != 5)
            break;
    ...
    case 19: {
        if (!pv_game_controller_ptr || *(uint32_t*)(pv_game_controller_ptr + 0x10) != 1)
            break;
    ...
    case 20: {
        if (!data.pv_data.play || data.pv_data.pv_end
            || !pv_game_controller_ptr || *(uint32_t*)(pv_game_controller_ptr + 0x10) == 2)
            state_old = 21;
    } break;
    case 21: {
        data.unload();
...
#pragma warning(disable: 26495)
        ...
        static_assert(sizeof(module_data) == 0xC0, "\"module_data\" struct should have a size of 0xC0");
#pragma warning(pop)

        static size_t(FASTCALL* module_data_handler_data_get)() = (size_t(FASTCALL*)()) 0x0000000140441B60;
        static bool (FASTCALL* module_data_handler__get_module_by_id)(size_t This, int32_t id, module_data* data)
            = (bool (FASTCALL*)(size_t This, int32_t id, module_data * data)) 0x00000001404421A0;
...
x_pv_game_firstread::x_pv_game_firstread() : init(), firstread() {
    enum language_enum {
        LANGUAGE_JAPANESE     = 0x00,
        LANGUAGE_ENGLISH      = 0x01,
        LANGUAGE_SIMP_CHINESE = 0x02,
        LANGUAGE_KOREAN       = 0x04,
        LANGUAGE_TRAD_CHINESE = 0x03,
        LANGUAGE_FRENCH       = 0x05,
        LANGUAGE_ITALIAN      = 0x06,
        LANGUAGE_GERMAN       = 0x07,
        LANGUAGE_SPANISH      = 0x08,
        LANGUAGE_MAX          = 0x09,
    };

    static language_enum(FASTCALL * curr_language_sel_get)()
        = (language_enum(FASTCALL*)())0x00000001402C8D20;
    language_enum lang = curr_language_sel_get();
    switch (lang) {
    case LANGUAGE_JAPANESE:
    case LANGUAGE_SIMP_CHINESE:
    default:
        lang_sel = 0;
        break;
