...
const pvpp* pvpp::read(const void* data, size_t size) {
    ...
    if (pp->chara_array) {
        ...
        const int32_t num_chara = pp->num_chara;
        for (int32_t i = 0; i < num_chara; i++) {
            ...
            if (chr->auth_3d_array) {
                ...
                const int32_t num_auth_3d = chr->num_auth_3d;
            ...
            if (chr->glitter_array) {
                ...
                const int32_t num_glitter = chr->num_glitter;
            ...
            if (chr_eff) {
                ...
                if (chr_eff->auth_3d_array) {
                    ...
                    const int32_t num_auth_3d = chr_eff->num_auth_3d;
                    for (int32_t i = 0; i < num_auth_3d; i++) {
    ...
    if (pp->effect_array) {
        ...
        const int32_t num_effect = pp->num_effect;
        for (int32_t i = 0; i < num_effect; i++) {
            ...
            if (eff->auth_3d_array) {
                ...
                const int32_t num_auth_3d = eff->num_auth_3d;
                for (int32_t i = 0; i < num_auth_3d; i++) {
            ...
            if (eff->glitter_array) {
                ...
                const int32_t num_glitter = eff->num_glitter;
                for (int32_t i = 0; i < num_glitter; i++) {
