...
namespace Glitter {
    ...
    void SceneX::CalcDisp(render_data_context& rend_data_ctx) {
        if (flags & SCENE_NOT_DISP)
            return;

        for (SceneEffectX& i : effects)
            if (i.ptr && i.disp)
                i.ptr->CalcDisp(rend_data_ctx);
        ...
        if (disp)
            rend_data_ctx.state.write_buffer(effect_group->buffers[GPMX_VAL->buffer_index],
                effect_group->buffer, effect_group->max_count * sizeof(Buffer));
#endif
    }
    ...
    void SceneX::Disp(render_data_context& rend_data_ctx, cam_data& cam, DispType disp_type, bool reflect) {
        if (flags & SCENE_NOT_DISP)
            return;

        for (SceneEffectX& i : effects)
            if (i.ptr && i.disp)
                i.ptr->Disp(rend_data_ctx, cam, disp_type, reflect);
    }
