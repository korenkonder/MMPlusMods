...
namespace Glitter {
    ...
    bool FileReaderX::CheckInit() {
        ...
        if (!effect_group->buffer_init) {
            ...
            if (max_count) {
                ...
                const size_t size = sizeof(Buffer) * max_count;

                for (p_dx_buffer& i : effect_group->buffers)
                    i.create(DX_BUFFER_VERTEX, (int32_t)size);

                Buffer* buffer = effect_group->buffer;
                p_dx_buffer* buffers = effect_group->buffers;
                size_t offset = 0;
                ...
                for (EffectX*& i : effect_group->effects) {
                    ...
                    for (EmitterX*& j : i->emitters) {
                        ...
                        for (ParticleX*& k : j->particles) {
                            ...
                            for (p_dx_buffer& m : k->buffers)
                                m = buffers[&m - k->buffers];

                            k->buffer_offset = (int32_t)offset;

                            buffer += k->max_count;
                            offset += sizeof(Buffer) * (size_t)k->max_count;
                        }
            ...
            for (EffectX*& i : effect_group->effects) {
                ...
                for (EmitterX*& j : i->emitters) {
                    ...
                    for (ParticleX*& k : j->particles) {
                        if (!k || k->data.type == PARTICLE_MESH || k->max_count <= 0 || init_buffers <= 0)
                            continue;

                        const size_t size = sizeof(Buffer) * k->max_count;
                        for (p_dx_buffer& l : k->buffers)
                            l.create(DX_BUFFER_VERTEX, (int32_t)size);
    ...
    bool FileReaderX::ParseDivaEffect(f2_struct* st) {
        ...
        if (!ParseEffectGroup(st, &eff_group->effects, eff_group)) {
            ...
            for (MeshX& i : eff_group->meshes)
                if (i.object_set != -1) {
                    objset_info_storage_load_set(i.object_set, -1);
                    i.load = true;
                }
    ...
    bool FileReaderX::UnpackDivaResource(f2_struct* st, EffectGroupX* eff_group) {
        ...
        for (size_t i = 0; i < count; i++)
            ids[i] = 0x2A000000 | (*(uint32_t*)(0x00000001416E4A70 + 0xF0))++;

        txp_set_load(st->data.data(), st->data.size(), &eff_group->resources, (uint32_t*)ids);
        ...
        for (EffectX*& i : eff_group->effects) {
            ...
            for (EmitterX*& j : i->emitters) {
                ...
                for (ParticleX*& k : j->particles) {
                    ...
                    k->data.texture.release();
                    k->data.mask_texture.release();
    ...
    bool FileReaderX::UnpackParticle(void* data, ParticleX* ptcl,
        ...
        ptcl->data.texture.release();
        ptcl->data.mask_texture.release();
